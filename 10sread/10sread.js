...
[task_local]
#10s阅读
0 8-14/1 * * * https://raw.githubusercontent.com/ugg999999/other/main/10sread.js, tag=10s阅读, img-url=https://raw.githubusercontent.com/ugg999999/other/main/10sread/10sread.png, enabled=true
[rewrite_local]
#10s阅读
.*read_channel\/do_read&pageshow.* url script-request-header https://raw.githubusercontent.com/ugg999999/other/main/10sread.js

#loon
http-request .*read_channel\/do_read&pageshow.* script-path=https://raw.githubusercontent.com/ugg999999/other/main/10sread.js, requires-body=true, timeout=10, tag=10s阅读

#surge

10s阅读 = type=http-request,pattern=.*read_channel\/do_read&pageshow.*,requires-body=1,max-size=0,script-path=https://raw.githubusercontent.com/ugg999999/other/main/10sread.js,script-update-interval=0
        ...
        $.get(options, async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    let text = html("div.rrp-box1-left-times").text();
                    let name = html("div.rrp-box1 p").text();
                    $.message =(name + " " + text).trim();
                    console.log($.message);
                }
            } catch (e) {
                $.logErr(e, resp);
