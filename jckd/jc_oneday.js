...
*/
const $ = new Env("晶彩看点每日任务");
const notify = $.isNode() ? require('./sendNotify') : '';
message = ""

let jc_onedaybody= $.isNode() ? (process.env.jc_onedaybody ? process.env.jc_onedaybody : "") : ($.getdata('jc_onedaybody') ? $.getdata('jc_onedaybody') : "")
let jc_onedaybodyArr = []
let jc_onedaybodys = ""
...
if (!jc_onedaybody) {
     $.msg($.name, '【提示】请点击每日任务某一任务获取body', '不知道说啥好', {
         "open-url": "给您劈个叉吧"
     });
     $.done()
 }
 else if (jc_onedaybody.indexOf("&") == -1) {
            jc_onedaybodyArr.push(jc_onedaybody)
 }
 else if (jc_onedaybody.indexOf("&") > -1) {
            jc_onedaybodys = jc_onedaybody.split("&")
 }
 else if (process.env.jc_onedaybody && process.env.jc_onedaybody.indexOf('&') > -1) {
            jc_onedaybodyArr = process.env.jc_onedaybody.split('&');
            console.log(`您选择的是用"&"隔开\n`)
 }
 else {
            jc_onedaybodys = [process.env.jc_onedaybody]
 };
    Object.keys(jc_onedaybodys).forEach((item) => {
        if (jc_onedaybodys[item]) {
            jc_onedaybodyArr.push(jc_onedaybodys[item])
        }
    })

!(async () => {
     if (typeof $request !== "undefined") {
     await getjc_onedaybody()
     $.done()
 }else{
    console.log(`共${jc_onedaybodyArr.length}个每日任务body`)
	        for (let k = 0; k < jc_onedaybodyArr.length; k++) {

                jc_onedaybody1 = jc_onedaybodyArr[k];
                console.log(`--------第 ${k + 1} 次每日任务激活执行中--------\n`)
                    await lookStart()
                await $.wait(1000);
                console.log("\n\n")
            }
	        ...
	        for (let k = 0; k < jc_cookieArr.length; k++) {
                ...
                console.log(`--------第 ${k + 1} 个账号每日任务上方宝箱奖励执行中--------\n`)
                for(let k = 0; k < 3; k++){
                    id = k.toString()
...
//获取每日任务激活body
async function getjc_onedaybody() {
if ($request.url.match(/\/ant.xunsl.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (jc_onedaybody) {
            if (jc_onedaybody.indexOf(bodyVal) > -1) {
                $.log("此每日任务任务请求已存在，本次跳过")
            } else if (jc_onedaybody.indexOf(bodyVal) == -1) {
                jc_onedaybodys = jc_onedaybody + "&" + bodyVal;
                $.setdata(jc_onedaybodys, 'jc_onedaybody');
                $.log(`${$.name}获取每日任务任务: 成功, jc_onedaybodys: ${bodyVal}`);
                bodys = jc_onedaybodys.split("&")
                // $.msg($.name, "获取第" + bodys.length + "个每日任务任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'jc_onedaybody');
            $.log(`${$.name}获取每日任务任务: 成功, jc_onedaybodys: ${bodyVal}`);
            $.msg($.name, `获取第一个每日任务任务请求: 成功🎉`, ``)
        }
    }
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活每日任务任务失败')
                    smbody = $.getdata('jc_onedaybody').replace(jc_onedaybody1 + "&", "");
                    $.setdata(smbody, 'jc_onedaybody');
                    console.log("该每日任务任务已自动删除")
                }
            } catch (e) {
            } finally {
                resolve()
