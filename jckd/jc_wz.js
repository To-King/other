...
let jc_wzbody= $.isNode() ? (process.env.jc_wzbody ? process.env.jc_wzbody : "") : ($.getdata('jc_wzbody') ? $.getdata('jc_wzbody') : "")
let jc_wzbodyArr = []
let jc_wzbodys = ""
...
const wzheader = {
 ...
 else if (jc_wzbody.indexOf("&") == -1) {
            jc_wzbodyArr.push(jc_wzbody)
 }
 else if (jc_wzbody.indexOf("&") > -1) {
            jc_wzbodys = jc_wzbody.split("&")
 }
 else if (process.env.jc_wzbody && process.env.jc_wzbody.indexOf('&') > -1) {
            jc_wzbodyArr = process.env.jc_wzbody.split('&');
            console.log(`您选择的是用"&"隔开\n`)
 }
 else {
            jc_wzbodys = [process.env.jc_wzbody]
 };
    Object.keys(jc_wzbodys).forEach((item) => {
        if (jc_wzbodys[item]) {
            jc_wzbodyArr.push(jc_wzbodys[item])
        }
    })
...
!(async () => {
    if (typeof $request !== "undefined") {
     ...
     $.done()
        ...
        console.log(`共${jc_wzbodyArr.length}个阅读body`)
        for (let k = 0; k < jc_wzbodyArr.length; k++) {
            // $.message = ""
            jc_wzbody1 = jc_wzbodyArr[k];
            // console.log(`${jc_wzbody1}`)
...
function getjc_wzbody() {
    if ($request.url.match(/\/ant.xunsl.com\/v5\/article\/info.json/)||$request.url.match(/\/ant.xunsl.com\/v5\/article\/detail.json/)) {
        ...
        if (jc_wzbody) {
            if (jc_wzbody.indexOf(bodyVal) > -1) {
                $.log("此阅读请求已存在，本次跳过")
            } else if (jc_wzbody.indexOf(bodyVal) == -1) {
                jc_wzbodys = jc_wzbody + "&" + bodyVal;
                $.setdata(jc_wzbodys, 'jc_wzbody');
                $.log(`${$.name}获取阅读: 成功, jc_wzbodys: ${bodyVal}`);
                bodys = jc_wzbodys.split("&")
                // $.msg($.name, "获取第" + bodys.length + "个阅读请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'jc_wzbody');
            $.log(`${$.name}获取阅读: 成功, jc_wzbodys: ${bodyVal}`);
