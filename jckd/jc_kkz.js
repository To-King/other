...
let jc_kkzbody= $.isNode() ? (process.env.jc_kkzbody ? process.env.jc_kkzbody : "") : ($.getdata('jc_kkzbody') ? $.getdata('jc_kkzbody') : "")
let jc_kkzbodyArr = []
let jc_kkzbodys = ""
...
if (!jc_kkzbody) {
 ...
 else if (jc_kkzbody.indexOf("&") == -1) {
            jc_kkzbodyArr.push(jc_kkzbody)
 }
 else if (jc_kkzbody.indexOf("&") > -1) {
            jc_kkzbodys = jc_kkzbody.split("&")
 }
 else if (process.env.jc_kkzbody && process.env.jc_kkzbody.indexOf('&') > -1) {
            jc_kkzbodyArr = process.env.jc_kkzbody.split('&');
            console.log(`您选择的是用"&"隔开\n`)
 }
 else {
            jc_kkzbodys = [process.env.jc_kkzbody]
 };
    Object.keys(jc_kkzbodys).forEach((item) => {
        if (jc_kkzbodys[item]) {
            jc_kkzbodyArr.push(jc_kkzbodys[item])
        }
    })
...
!(async () => {
    ...
    console.log(`共${jc_kkzbodyArr.length}个看看赚body`)
	        for (let k = 0; k < jc_kkzbodyArr.length; k++) {

                jc_kkzbody1 = jc_kkzbodyArr[k];
...
//获取看看赚激活body
async function getjc_kkzbody() {
if ($request.url.match(/\/ant.xunsl.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (jc_kkzbody) {
            if (jc_kkzbody.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (jc_kkzbody.indexOf(bodyVal) == -1) {
                jc_kkzbodys = jc_kkzbody + "&" + bodyVal;
                $.setdata(jc_kkzbodys, 'jc_kkzbody');
                $.log(`${$.name}获取看看赚任务: 成功, jc_kkzbodys: ${bodyVal}`);
                bodys = jc_kkzbodys.split("&")
                // $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'jc_kkzbody');
            $.log(`${$.name}获取看看赚任务: 成功, jc_kkzbodys: ${bodyVal}`);
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('jc_kkzbody').replace(jc_kkzbody1 + "&", "");
                    $.setdata(smbody, 'jc_kkzbody');
