const $ = new Env("晶彩看点视频");
const notify = $.isNode() ? require('./sendNotify') : '';
message = ""


let jc_kspbody= $.isNode() ? (process.env.jc_kspbody ? process.env.jc_kspbody : "") : ($.getdata('jc_kspbody') ? $.getdata('jc_kspbody') : "")
let jc_kspbodyArr = []
let jc_kspbodys = ""
...
const wzheader = {
  ...
  if (!jc_timebody) {
     $.msg($.name, '【提示】请点击视频获取body，再跑一次脚本', '不知道说啥好', {
         "open-url": "给您劈个叉吧"
     });
     $.done()
 }
...
/* if (!jc_kspbody) {
 ...
 else if (jc_kspbody.indexOf("&") == -1) {
            jc_kspbodyArr.push(jc_kspbody)
 }
 else if (jc_kspbody.indexOf("&") > -1) {
            jc_kspbodys = jc_kspbody.split("&")
 }
 else if (process.env.jc_kspbody && process.env.jc_kspbody.indexOf('&') > -1) {
            jc_kspbodyArr = process.env.jc_kspbody.split('&');
            console.log(`您选择的是用"&"隔开\n`)
 }
 else {
            jc_kspbodys = [process.env.jc_kspbody]
 };
    Object.keys(jc_kspbodys).forEach((item) => {
        if (jc_kspbodys[item]) {
            jc_kspbodyArr.push(jc_kspbodys[item])
        }
    })
...
!(async () => {
    if (typeof $request !== "undefined") {
     ...
     $.done()
        ...
        console.log(`共${jc_kspbodyArr.length}个视频body`)
        for (let k = 0; k < jc_kspbodyArr.length; k++) {
            // $.message = ""
            jc_kspbody1 = jc_kspbodyArr[k];
            // console.log(`${jc_kspbody1}`)
            console.log(`--------第 ${k + 1} 次视频任务执行中--------\n`)
            await wzjl()
            await $.wait(60000);
        ...
        //     if (message.length != 0) {
        //            await notify.sendNotify("晶彩看点视频视频", `${message}\n\n shaolin-kongfu`);
        //     }
        // } else {
        //     $.msg($.name, "",  message)
        // }
...
function getjc_kspbody() {
    if ($request.url.match(/\/ant.xunsl.com\/v5\/article\/info.json/)||$request.url.match(/\/ant.xunsl.com\/v5\/article\/detail.json/)) {
        ...
        if (jc_kspbody) {
            if (jc_kspbody.indexOf(bodyVal) > -1) {
                $.log("此视频请求已存在，本次跳过")
            } else if (jc_kspbody.indexOf(bodyVal) == -1) {
                jc_kspbodys = jc_kspbody + "&" + bodyVal;
                $.setdata(jc_kspbodys, 'jc_kspbody');
                $.log(`${$.name}获取视频: 成功, jc_kspbodys: ${bodyVal}`);
                bodys = jc_kspbodys.split("&")
                // $.msg($.name, "获取第" + bodys.length + "个视频请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'jc_kspbody');
            $.log(`${$.name}获取视频: 成功, jc_kspbodys: ${bodyVal}`);
            $.msg($.name, `获取第一个视频请求: 成功🎉`, ``)
        }
    }
...
function wzjl(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            headers : wzheader,
            body : jc_kspbody1,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {

                const result = JSON.parse(data)
                if(result.items.read_score !== "undefined" ){
                    console.log('\n浏览视频成功，获得：'+result.items.read_score + '金币')
                }else{
                    console.log('\n看太久了，换一篇试试')
                }
            } catch (e) {
    ...
    if ($request.url.match(/\/ant.xunsl.com\/v5\/user\/stay.json/)) {
        ...
        if (jc_timebody) {
            if (jc_timebody.indexOf(bodyVal) > -1) {
                ...
                $.log(`${$.name}获取视频: 成功, jc_timebodys: ${bodyVal}`);
                bodys = jc_timebodys.split("&")
                $.msg($.name, "获取第" + bodys.length + "个视频时长请求: 成功🎉", ``)
            }
        } else {
            $.setdata($request.body,'jc_timebody');
            $.log(`${$.name}获取视频: 成功, jc_timebodys: ${bodyVal}`);
            $.msg($.name, `获取第一个视频时长请求: 成功🎉`, ``)
        }
    }
}
