...
https:\/\/www\.xiaodouzhuan\.cn\/jkd\/newMobileMenu\/infoMe\.action url script-request-body jkd.js
...
*/
const $ = new Env('聚看点')
const drawcash = $.getdata('jkd_cash') || "30" //提现金额
const wxname = $.getdata('jkd_name') || ""//微信真实名字，可以在双引号内填入
let CookieArr=[],BodyArr=[];
let bodys = $.getdata('jkd_body')
let signtimes = $.getdata('jkd_times')
let cashout = $.getdata('jkd_out')|| false
let UA = 'jkdDian/5.6.5 (iPhone; iOS 14.2; Scale/3.00)'
let taskresult = "",sumnotify ="";
let calendarpic = "";
if ($.isNode()) {
  if (process.env.jkd_BODY && process.env.jkd_BODY.indexOf('&') > -1) {
  JKbody = process.env.jkd_BODY.split('&');
  }
  else if (process.env.jkd_BODY && process.env.jkd_BODY.indexOf('\n') > -1) {
  JKbody = process.env.jkd_BODY.split('\n');
  } else {
  JKbody = process.env.jkd_BODY.split()
  }
...
if (typeof $request !== 'undefined') {
   ...
   if(bodyVal)$.setdata(bodyVal,'jkd_body')
     $.log(`bodyVal:${bodyVal}`)
     $.msg($.name,"获取请求成功")
     $.done()
   }
  ...
  return new Promise((resolve, reject) =>{
   ...
   try{
     ...
     if (get_sign.ret == "ok"){
      ...
      if(signtimes) $.setdata(signtimes, 'jkd_times')
         $.sub = `签到成功🎉`
